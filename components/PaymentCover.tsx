...
import { Printer, Settings, FileText, Plus, Trash2 } from 'lucide-react';

interface Props {
  sheets: MeasurementSheet[];
  previousQuantities: Record<string, number>;
  projectInfo: ProjectInfo;
  coverData: CoverData; // YENİ
  setCoverData: React.Dispatch<React.SetStateAction<CoverData>>; // YENİ
}
...
export const PaymentCover: React.FC<Props> = ({ sheets, previousQuantities, projectInfo, coverData, setCoverData }) => {
  ...
  // 1. Yapılan İşler Tutarı (Otomatik Hesaplanan)
  ...
  // B) Toplam Kesinti
  // (Manuel girilen kesintiler doğrudan düşülür)
    ...
    <div className="bg-white p-4 md:p-8 rounded-xl shadow-sm min-h-screen font-sans">
      ...
      <div className="flex justify-between items-start mb-6 no-print">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowSettings(!showSettings)} className="flex items-center gap-2 bg-gray-100 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-200 transition">
            <Settings size={18} /> Ayarlar
          </button>
          <button onClick={() => window.print()} className="flex items-center gap-2 bg-blue-800 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition shadow-lg">
      ...
      {/* --- ANA TABLO --- */}
      <div className="border-2 border-black max-w-5xl mx-auto text-sm">
            ...
            <div key={row.id} className="grid grid-cols-12 border-b border-gray-300 hover:bg-red-50 group">
              <div className="col-span-4 p-1 border-r border-black pl-4 flex items-center">
                ...
                <input 
                  ...
                  placeholder="Kesinti Açıklaması (Örn: Stopaj %3)"
                  className="w-full bg-transparent outline-none text-sm placeholder-gray-300"
                />
